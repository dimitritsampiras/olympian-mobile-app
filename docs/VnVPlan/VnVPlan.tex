...
\begin{document}
	...
	\subsubsection{Usability and Humanity Tests}
		...
		Survey: [\ref{learnability-survey}]
		...
		Survey: [\ref{understandability-survey}]
	...
	\subsubsection{Security Tests}
		...
		Input: On attempt 1, a password string that is registered, and on attempt 2, an unregistered password.
		
		Output: The inputted password is tested against hashed version of actual user password. For the valid case (attempt 1) - verification is successful and user is logged in. For the invalid case (attempt 2) - verification is unsuccessful and user is notified of unsuccessful login attempt.
		...
		Input/Condition: A valid data entry for a workout routine [\ref{WR-1-input}] is entered into the database.
	...
	\subsubsection{Legal Requirements Tests}
		...
		Output/Result: All usage and storage of user data is compliant with the Digital Privacy Act [\ref{digital-privacy-act}].
...
\begin{table}[h!]
	...
	The usability survey will be conducted by presenting a sample of at least 10 users with the mobile application, started from the home page. The users will then be asked to do their best to use the application naturally, using an unguided questionnaire technique. \\ \\
	The users will be asked to attempt completing a set of tasks. After attempting each task, the user will be asked to rate the application on a scale of 1 - 10 in a set of usability categories.
	
	\subsubsection{Use of survey results}
	The results of the survey will allow us to focus our development time into improving those areas of our application that users would most like to see improved. This can allow us to be efficient and precise with our improvements and targeted improvements.
	...
	\begin{enumerate}
		...
		\item Starting a workout
	\end{enumerate}
	
	\subsubsection{Categories}
	\begin{enumerate}
		\item Overall style (simplicity, cleanliness, aesthetic) \label{style-survey}
		\item Visibility of text and figures \label{visibility-survey}
		\item Overall learnability \label{learnability-survey}
		\item Overall understandability \label{understandability-survey}
		\item Rating of how audible / pleasant system sounds are \label{sound-survey}
		\item Colour visibility / contrast \label{colour-survey}
	\end{enumerate}

	\subsection{Digital Privacy Act} \label{digital-privacy-act}
	Here is where readers may learn more about Canada's \href{https://laws-lois.justice.gc.ca/eng/annualstatutes/2015_32/FullText.html}{Digital Privacy Act}, which Olympian will be using to enforce digital privacy for its users.
